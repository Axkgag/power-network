...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, class_agnostic=False):
    # prediction = [xc, yc, w, h, ang, obj_conf, ...]
    ...
    for i, image_pred in enumerate(prediction):
        ...
        # Detections ordered as (x1, y1, x2, y2, ang, obj_conf, class_conf, class_pred)
...
def convert_box(bbox_score):
    if len(bbox_score) == 7:
        score = float(bbox_score[5] * bbox_score[6])
    else:
        score = float(bbox_score[5])
    bbox = bbox_score[0:4]
    ang = bbox_score[4]
    result = np.array(
        [int((bbox[0] + bbox[2]) / 2), int((bbox[1] + bbox[3]) / 2), int(bbox[2] - bbox[0]), int(bbox[3] - bbox[1]),
         ang])
