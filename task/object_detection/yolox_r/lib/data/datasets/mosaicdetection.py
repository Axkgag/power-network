...
def vis(img, bboxes):
    for bbox in bboxes:
        ...
        cv2.line(img, (int(x1n), int(y1n)), (int(x2n), int(y2n)), (255, 0, 0), 1, 4)
        cv2.line(img, (int(x2n), int(y2n)), (int(x3n), int(y3n)), (0, 0, 255), 1, 4)
        cv2.line(img, (int(x0n), int(y0n)), (int(x3n), int(y3n)), (255, 0, 0), 1, 4)
    return img
...
class MosaicDetection(Dataset):
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            for i_mosaic, index in enumerate(indices):
                img, _labels, _, img_id, _ = self._dataset.pull_item(index)
            ...
            #     "D:/task/ChipCounter/data/chip/mosaic", time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
            # )
            # os.makedirs(save_folder, exist_ok=True)
            # save_file_name = os.path.join(save_folder, str(img_id.item())+".jpg")
            # img = mix_img.transpose(1, 2, 0)
            # img = vis(img, padded_labels[:, 1:])
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        ang_labels = cp_labels[:, 4:5].copy()
        cls_labels = cp_labels[:, 5:6].copy()
        box_labels = np.hstack((cp_bboxes_transformed_np, ang_labels))
