...
def createInstance(config_path=None):
    predictor = LSKPredictor()
    return predictor

class LSKPredictor(AbstractObjectDetectPredictor):
    ...
    def run_eval(self, val_img, val_ann, data_dir):
        ...
        if not os.path.exists(val_ann):
            print("annotations.json is not exist !")
            return
        ...
        *_, summary = evaluator.evaluate(
            model, is_distributed, args.fp16, trt_file, decoder, self.test_size
        )
        logger.info("\n" + summary)
