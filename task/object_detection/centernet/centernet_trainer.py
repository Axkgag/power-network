...
from riemann_interface.defaults import  AbstractTrainer,Status,TrainStatus,AbstractSender, AbstractObjectDetectTrainer
...
class CenterNetTrainer(AbstractObjectDetectTrainer):
    ...
    def setupInfo(self, train_dir: str):
        ...
        with open(trainer_config_path,'r',encoding='utf-8') as f:
            ...
            train_config["trainval_dir"] = train_dir
    ...
    def testTrainDataset(self, save_dir, train_mode):
        ...
        if train_mode == "train":
            train_loader = torch.utils.data.DataLoader(
                self.Dataset(self.opt, "trainval"),
                batch_size=1,
                shuffle=False,
                num_workers=0,
                pin_memory=False
            )
            coco_anno = coco.COCO(self.config["trainval_annot_path"])
        elif train_mode == "val":
            train_loader = torch.utils.data.DataLoader(
                self.Dataset(self.opt, "val"),
                batch_size=1,
            ...
            coco_anno = coco.COCO(self.config["val_annot_path"])
        elif train_mode == "test":
            train_loader = torch.utils.data.DataLoader(
                self.Dataset(self.opt, "test"),
                batch_size=1,
                shuffle=False,
                num_workers=0,
                pin_memory=False
            )
            coco_anno = coco.COCO(self.config["test_annot_path"])
    ...
    def exportModel(self, export_path: str):
        ...
        if self.config["test_img_dir"] != self.config["train_img_dir"]:
            ...
            datasets.append(3)
        # stats=self.testTrainDataset(save_dir, train_mode="train")
        ...
        if os.path.exists(os.path.join(self.train_dir, "trainer.json")):
            with open(os.path.join(export_path, "predictor.json"), "w+", encoding='utf-8') as write_f:
                tr_json = json.load(open(os.path.join(self.train_dir, "trainer.json"), "r", encoding='utf-8'))
                pred_json = dict()
