...
class DistributionUncertainty(nn.Module):
    ...
    def __init__(self, p=0.3, eps=1e-6):
...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv, pertubration=None, uncertainty=0.0, **kwargs):
        self.inplanes = 64
        ...
        for head in sorted(self.heads):
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                        nn.Conv2d(256, head_conv, kernel_size=3, padding=1, bias=False),
                        nn.BatchNorm2d(head_conv),
                        nn.ReLU(inplace=True),
                        nn.Conv2d(head_conv, 64, kernel_size=3, padding=1, bias=False),
                        nn.BatchNorm2d(64),
                        nn.ReLU(inplace=True),
    ...
    def forward(self, x, label=None):
        ...
        for head in self.heads:
            ret[head] = self.__getattr__(head)(f)
        return [ret]
...
def get_pose_net(num_layers, heads, head_conv):
    ...
    model = PoseResNet(block_class, layers, heads, head_conv=head_conv, pertubrarion=DistributionUncertainty, uncertainty=0.5)
