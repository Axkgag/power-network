...
def _nms(heat, kernel=3):
    """
    将heat_map解码成bbox
    hmax用于寻找8近邻最大值点，keep为heat极大值点，除极大值点其余为0，极大值点为原值
    """
    pad = (kernel - 1) // 2
...
def _topk(scores, K=40):
    """
    topk最大的k个
    返回topk的值(scores),id(inds),类别?(clses),ys、xs(横纵坐标)
    """
    batch, cat, height, width = scores.size()
...
def ctdet_decode(heat, wh, ang, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
