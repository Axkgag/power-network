...
def fit(train_loader, 
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        # Train stage
        train_loss, metrics = train_epoch(train_loader, model, loss_fn, cls_fn, optimizer, cuda, log_interval, metrics)
        test_acc = test(val_loader, model, num_classes, cuda)

        scheduler.step()

        message = 'Epoch: {}/{}. Train set: Average loss: {:.4f} Test acc: {:.4f}'.format(epoch + 1, n_epochs, train_loss, test_acc)
...
def train_epoch(train_loader, model, loss_fn, cls_fn, optimizer, cuda, log_interval, metrics):
    ...
    for batch_idx, (img2, label, _, _) in enumerate(train_loader):
        ...
        if label is not None:
            loss_clses = cls_fn(scores, label)
